...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen flex bg-white relative overflow-hidden">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative z-10">
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600">Sign in to continue your amazing conversations</p>
          </div>

          <button onClick={handleGoogleSignIn} className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-200 text-gray-700 font-semibold py-3.5 rounded-xl hover:border-gray-300 hover:shadow-md transition-all mb-6">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" className="w-full pl-12 pr-12 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#5e17eb] focus:ring-2 focus:ring-[#5e17eb]/20 transition-all text-gray-900" />
            ...
            <div className="flex justify-between items-center">
              ...
              <Link href="/forgot-password" className="text-sm text-[#5e17eb] font-semibold hover:underline cursor-pointer">
                Forgot password?
              </Link>
            </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600">Don&apos;t have an account? <Link href="/signup" className="text-[#5e17eb] font-semibold hover:underline cursor-pointer">Sign up here</Link></p>
          </div>
        </motion.div>
      </div>

      {/* Right Side - Diagonal Masonry (Opposite side of signup) */}
      ...
      {/* Floating Badges - SCATTERED AROUND (different positions) */}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowWelcomeModal(false)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ type: 'spring', duration: 0.5 }} className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="text-center">
                ...
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowWelcomeModal(false)} className="w-full bg-gradient-to-r from-[#ff2e2e] to-[#5e17eb] text-white font-bold py-4 rounded-xl hover:shadow-lg transition-all">Continue Exploring</motion.button>
              </div>
            </motion.div>
          </motion.div>
        )}
